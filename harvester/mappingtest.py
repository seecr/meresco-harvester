...
class MappingTest(unittest.TestCase):
	def testCreateUploadFields(self):
		...
		uploadfields = dcmap.createUpload(TestRepository(), header, metadata).fields
...
upload.fields['title'] = 'Dit is een test'"""
		...
		dcmap = Mapping('mappingId')
		dcmap.code = testcode
		fields = dcmap.createUpload(TestRepository(),wrapp(''),None).fields
		self.assert_(fields)
...
"""
		...
		upload = datamap.createUpload(TestRepository(),header,metadata, logger)
...
"""
		header = parse_xml("""<header><identifier>oai:ident:321</identifier><datestamp>2005-08-29T07:08:09Z</datestamp></header>""").header
		metadata = parse_xml("""<metadata></metadata>""").metadata
		upload = datamap.createUpload(TestRepository(),header,metadata)
...
upload.fields['title'] = 'Dit is een test'"""
		...
		try:
			...
			datamap.createUpload(TestRepository(),header,None, logger, doAsserts=True)
			self.fail()
		...
		try:
			datamap.createUpload(TestRepository(),wrapp(''),None, doAsserts=True)
			self.fail()
		except DataMapAssertionException, ex:
			self.assertEquals('1 not equal 2', str(ex))
		...
		stream = StringIO()
		logger = StreamEventLogger(stream)
		datamap.createUpload(TestRepository(),wrapp(''),None, logger, doAsserts=False)
